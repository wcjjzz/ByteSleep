  ...
  return (
    <div className="mx-auto max-w-[1300px] space-y-8">
      ...
      <div className="grid gap-6 md:grid-cols-3">
        <StatCard title="今日新生成结果" value="8" sub="+3 较昨日" icon={FileText} accent="bg-blue-100/60" />
        <StatCard title="需高关注结果" value="2" sub="未读" icon={ShieldAlert} accent="bg-rose-100/60" />
        <StatCard title="近期复查更新" value="5" sub="本周" icon={LineChart} accent="bg-indigo-100/60" />
      </div>

      <div className="grid gap-8 lg:grid-cols-5">
        <div className="space-y-4 lg:col-span-3">
          <div className="flex items-center justify-between">
            <h2 className="inline-flex items-center gap-2 text-lg font-black text-slate-800"><Activity size={20} className="text-blue-500" /> 最新业务更新</h2>
            <span className="rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-bold text-slate-400">已过滤专属跟进名单</span>
          </div>
          {unreadPublicUpdates.length > 0 ? unreadPublicUpdates.map((update) => {
            ...
            <Card className="flex min-h-72 flex-col items-center justify-center text-center text-slate-400">
              ...
              <p className="font-bold text-slate-500">公共流中暂无新的业务动态</p>
              <p className="mt-2 text-sm">关注患者的动态已单独收纳到右侧面板</p>
            </Card>
          )}
        </div>
        ...
        <div className="lg:col-span-2">
          <Card className="flex h-[560px] flex-col overflow-hidden p-0">
            <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50/70 px-6 py-5">
              <h2 className="inline-flex items-center gap-2 text-lg font-black text-slate-800"><Star size={18} className="fill-amber-400 text-amber-400" /> 专属跟进动态</h2>
            ...
            <div className="flex-1 space-y-3 overflow-y-auto p-4">
              {unreadFollowedUpdates.length > 0 ? unreadFollowedUpdates.map((update) => {
                ...
                <div className="flex h-full flex-col items-center justify-center text-center text-slate-400">
                  ...
                  <p className="mt-2 text-sm">您关注患者的新报告和状态变化将在这里优先提醒</p>
                </div>
              )}
            </div>
          </Card>
